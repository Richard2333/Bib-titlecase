...
cus_remain = ['title','author', 'booktitle', 'journal', 'year', 'pages', 'volume', 'number','note','archivePrefix'] # reserved attributes, can be customized
fix = ['@']
...
for line in ori_bib:
    ...
    if not any([in_line(t,com_line) for t in remain]) and line != '}\n' and line != '}':
        ...
        new_title = '{'+titlecase(title)+'}' #upper_all_tokens(title)
